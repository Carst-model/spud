...
class UseView(gtk.Window):
    ...
    def __set_treestore(self, node):
        def set_treestore(node, iter, type):
            if node.tag == RELAXNG + "element":
                ...
                self.mapping[self.tree.getpath(
                    node)] = self.treestore.get_path(child_iter)
                type = None
                ...
                if query not in cache:
                    cache[query] = self.tree.xpath(query,
                                                   namespaces={'t':
                                                               RELAXNGNS})[0]
                node = cache[query]
                child_iter = iter
    ...
    def __update(self, schema, paths):
        ...
        start = self.tree.xpath('/t:grammar/t:start',
                                namespaces={'t': RELAXNGNS})[0]
        self.mapping = {}
        ...
        t = threading.Thread(target=async_update,
                             args=(self, start, schema, paths,
                                   self.statusbar.get_context_id("update")))
        t.start()
