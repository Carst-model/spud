...
class Tree(gobject.GObject):
    ...
    __gsignals__ = {
        "on-set-data":
        (gobject.SignalFlags.RUN_LAST, gobject.TYPE_NONE, (str, )),
        "on-set-attr":
        (gobject.SignalFlags.RUN_LAST, gobject.TYPE_NONE, (str, str))
    }
    ...
    """
        ...
        if not invalid and isinstance(new_data, str) and new_data != "":
            if new_data != data and self.validity_check(datatype,
                                                        new_data) is None:
                return None
            else:
                return new_data
    ...
    def copy(self):
        new_copy = Tree()
        for attr in [
                "attrs", "name", "schemaname", "doc", "cardinality",
                "datatype", "data", "active", "valid"
        ]:
        ...
        for tree in treelist:
            ...
            for tree in outlist:
                if tree.cardinality in ['+', '*']:
                    inactive_list = [
                        x for x in outlist if x.schemaname == tree.schemaname
    ...
    def write(self, filename):
        ...
        xmlTree = etree.tostring(self.write_core(None),
                                 pretty_print=True,
    ...
    """
        return self.is_comment() or self.name in [
            "integer_value", "real_value", "string_value", "logical_value"
        ]
    ...
    def get_mixed_data(self):
        integers = [
            child for child in self.children if child.name == "integer_value"
        ]
        reals = [
            child for child in self.children if child.name == "real_value"
        ]
        logicals = [
            child for child in self.children if child.name == "logical_value"
        ]
        strings = [
            child for child in self.children if child.name == "string_value"
        ]
    ...
    def is_sliceable(self):
        ...
        return (self.datatype is not None
                and self.datatype != "fixed") or self.attrs
