...
class AttrIterator():
    ...
    def next(self):
        ...
        while len(self.attributes) != 0 and self.attributes[
                0].start_index == self.start_index:
            if self.attributes[0].end_index > self.start_index:
                self.attribute_stack.append(self.attributes[0])
                self.end_index = min(self.end_index,
                                     self.attributes[0].end_index)
            self.attributes = self.attributes[1:]
        if len(self.attributes) > 0:
            self.end_index = min(self.end_index,
                                 self.attributes[0].start_index)
        return True
...
class PangoBuffer(gtk.TextBuffer):
    ...
    def set_text(self, txt):
        ...
        try:
            success, self.parsed, self.txt, self.separator = pango.parse_markup(
                txt, -1, '\x00')
        except:
...
class InteractivePangoBuffer(PangoBuffer):
    def __init__(self, normal_button=None, toggle_widget_alist=[]):
        ...
        if normal_button:
            normal_button.connect(
                'clicked', lambda *args: self.remove_all_tags())
        self.tag_widgets = {}
...
class SimpleEditor:
    def __init__(self):
        ...
        self.sw.add(self.tv)
        self.ipb = InteractivePangoBuffer(normal_button=self.nb)
        ...
        for lab, stock, font in [
            ('gtk-italic', True, '<i>italic</i>'),
        ...
        self.qb.connect(
            'clicked', lambda *args: self.w.destroy() or gtk.main_quit())
