...
class MixedTree:
    ...
    """
        ...
        self.attrs = dict(
            list(self.parent.attrs.items()) + [
                x for x in list(self.child.attrs.items())
                if x[0] not in excluded_attrs
            ])
    ...
    def matches(self, text, case_sensitive=False):
        ...
        if not self.child.data is None and not text_re.search(
                self.child.data) is None:
            return True
        else:
            return False
    ...
    """
        ...
        if self.child.attrs["dim1"][0] != "fixed" or self.child.attrs["rank"][
                0] != "fixed":
            return False
        ...
        elif self.child.attrs["rank"][1] != "1" or self.child.attrs["shape"][
                0] is not int:
            return False

        # The data for the element must be a list of one or more
        if not isinstance(self.datatype,
                          plist.List) or self.datatype.cardinality != "+":
            return False
    ...
    """
        ...
        if "dim1" in list(self.child.attrs.keys()):
            ...
            if "dim2" in list(self.child.attrs.keys()):
                dim2 = int(
                    eval(self.child.attrs["dim2"][1], {"dim": dimension}))
    ...
    """
        ...
        return dim1 == dim2 and "symmetric" in list(self.child.attrs.keys(
        )) and self.child.attrs["symmetric"][1] == "true"
