...
class Schema(object):
    def __init__(self, schemafile):
        ...
        self.tree = etree.parse(io.BytesIO(preprocess.preprocess(schemafile)),
                                p)

        self.callbacks = {
            'element': self.cb_element,
    ...
    """
        ...
        for child in children:
            if self.tag(child) == "choice":
                out_children = out_children + self.choice_children(
...
#    sys.exit(1)
    ...
    def xml_read_merge(self, datatree, xmlnode):
        ...
        if datatree.__class__ is tree.Tree:
            ...
            if "name" in xmlkeys:
                ...
                possibles = [
                    tree_choice for tree_choice in datatree.get_choices()
                    if tree_choice.name == xmlnode.tag
                ]
            ...
            if schemachild.cardinality in ['', '?']:
                for curtree in schemachild.get_choices():
                    ...
                    if have_fixed_name is False:
                        ...
                        xml = xmlnode.xpath(name + '[@name="%s"]' %
                                            curtree.get_attr("name"))
    ...
    ###########################################################################################
    def assign_xml_children(self, priority_queue, xmlnode, xmls,
                            schemachildren, used, rootdoc):
        ...
        for schemachild in priority_queue:
            if schemachild.cardinality in ['', '?']:
                ...
                if len(xmls[schemachild.schemaname]) == 1:
                    ...
                    if xmldata.getroottree().getroot() != rootdoc.getroot():
                        self.xml_read_core(child.get_current_tree(), xmldata,
                                           xmldata.getroottree())
                        child.active = False
                        child.recurse = False
                else:
            ...
            if isinstance(
                    schemachild,
                    choice.Choice) and schemachild.cardinality in ['', '?']:
    ...
    def xml_read_core(self, datatree, xmlnode, rootdoc):
        ...
        bins = self.assign_xml_children(priority_queue, xmlnode, xmls,
                                        schemachildren, used, rootdoc)
