...
#         self.scherror.on_validate()
    ...
    def is_node_editable(self):
        return (self.node is not None and self.node.active
                ...
                and (not self.node.is_tensor(self.geometry_dim_tree)
                     or self.geometry_dim_tree.data is not None))
    ...
    """
        ...
        if self.frame.get_child() is not None:
            if isinstance(self.data, gtk.TextView) or isinstance(
                    self.data, gtksource.View):
    ...
    """
        ...
        scrolledWindow.get_child().set_property("shadow-type",
                                                gtk.ShadowType.NONE)
            ...
            for j in range(dim2):
                ...
                if not is_symmetric or i >= j:
                    ...
                    if self.node.data is None:
                        ...
                        entry.modify_text(gtk.StateFlags.NORMAL,
    ...
    """
        ...
        if self.node.data is None:
            ...
            self.data.get_child().modify_text(gtk.StateFlags.NORMAL,
                                              gdk.color_parse("blue"))
            self.data.get_child().modify_text(gtk.StateFlags.PRELIGHT,
                                              gdk.color_parse("blue"))
    ...
    """
        ...
        new_data = self.data.get_buffer().get_text(
            *self.data.get_buffer().get_bounds(), include_hidden_chars=True)
    ...
    """
        ...
        if self.node.data is None and not self.interacted:
            ...
            new_data = self.node.validity_check(self.node.datatype[1],
                                                new_data)
            if new_data is None:
                return False
    ...
    """
        ...
        if (self.node is not None and self.node.datatype is not None
                and not self.node.is_tensor(self.geometry_dim_tree)
                and self.node.data is None and not self.interacted):
            self.data.get_buffer().set_text("")
