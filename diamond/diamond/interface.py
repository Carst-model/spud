...
from . import (TextBufferMarkup, attributewidget, choice, commentwidget,
               config, databuttonswidget, datatype, datawidget, debug,
               descriptionwidget, dialogs, diffview, mixedtree, plist, plugins,
               schema, scherror, sliceview, tree, useview)
...
class Diamond:
    def __init__(self,
        ...
        self.scherror = scherror.DiamondSchemaError(self, gladefile,
                                                    schemafile,
                                                    schematron_file)
        ...
        signals = {
            ...
            "on_display_properties_toggled":
            self.on_display_properties_toggled,
        ...
        if schematron_file is None:
            ...
            menu.get_children()[3].get_submenu().get_children(
            )[1].set_property("sensitive", False)
    ...
    def load_schema(self, schemafile):
        ...
        if schemafile == self.schemafile:
            self.statusbar.set_statusbar('Schema ' + schemafile +
                                         ' already loaded')
            return
        ...
        try:
            ...
            dialogs.error_tb(
                self.main_window,
                "Unable to open schema file \"" + schemafile + "\"")
    ...
    def display_validation_errors(self, errors):
        ...
        if len(lost_eles) > 0 or len(added_eles) > 0 or len(
                lost_attrs) > 0 or len(added_attrs) > 0:
            saved = False
            msg = ""
            if len(lost_eles) > 0:
    ...
    def load_file(self, filename):
        ...
        try:
            ...
            if tree_read is None:
                dialogs.error_tb(self.main_window,
    ...
    """
        ...
        if self.suffix is None:
            for xmlname in config.schemata:
                filter_names_and_patterns[config.schemata[xmlname]
                                          [0]] = "*." + xmlname
        elif self.suffix in list(config.schemata.keys()):
            filter_names_and_patterns[config.schemata[self.suffix]
                                      [0]] = "*." + self.suffix
        else:
    ...
    """
        ...
        if self.filename is None:
            ...
            self.main_window.get_window().set_cursor(
                gdk.Cursor(gdk.CursorType.WATCH))
            try:
                self.tree.write(self.filename)
            except:
                dialogs.error_tb(self.main_window,
                                 "Saving to \"" + self.filename + "\" failed")
                self.statusbar.clear_statusbar()
    ...
    def on_display_properties_toggled(self, widget=None):
        ...
        optionsFrame.set_property("visible",
                                  not optionsFrame.get_property("visible"))
        return
   ...
   """
    ...
    """
        ...
        about.set_authors([
            "Patrick E. Farrell", "James R. Maddison", "Matthew T. Whitworth",
            "Fraser J. Waters"
        ])
        about.set_license(
            "Diamond is free software: you can redistribute it and/or modify\n"
    ...
    def on_paste(self, widget=None):
        ...
        if node != None:
            ...
            self.treeview.freeze_child_notify()
            iter = self.set_treestore(self.selected_iter, [newnode], True,
                                      True)
    ...
    def __diff(self, path):
        def run_diff(self, path):
            ...
            self.statusbar.set_statusbar("Diff calculated (took " +
                                         str(seconds) + " seconds)")
            return False
        ...
        self.statusbar.set_statusbar(
            "Calculating diff... (this may take a while)")
    ...
    """
        ...
        imgcolumn.set_cell_data_func(cellPicture,
                                     self.set_cellpicture_cardinality)
    ...
    """
        ...
        for t in new_tree:
            ...
            if isinstance(t, tree.Tree):
                if t.is_hidden():
                    attrid = t.connect("on-set-attr", self.on_set_attr,
                                       self.treestore.get_path(iter))
                    dataid = t.connect("on-set-data", self.on_set_data,
                                       self.treestore.get_path(iter))
                ...
                if replace:
                    child_iter = self.treestore.insert_before(
                        iter, replacediter, [t, t, liststore])
                else:
                    child_iter = self.treestore.append(iter, [t, t, liststore])

                attrid = t.connect("on-set-attr", self.on_set_attr,
                                   self.treestore.get_path(child_iter))
                dataid = t.connect("on-set-data", self.on_set_data,
                                   self.treestore.get_path(child_iter))
                self.signals[t] = (attrid, dataid)

                if recurse and t.active:
                    self.set_treestore(child_iter, t.children, recurse)
                ...
                if replace:
                    child_iter = self.treestore.insert_before(
                        iter, replacediter, [t, ts_choice, liststore])
                else:
                    child_iter = self.treestore.append(
                        iter, [t, ts_choice, liststore])
                ...
                if recurse and t.active:
                    self.set_treestore(child_iter, ts_choice.children, recurse)
    ...
    """
        ...
        while child_iter is not None:
            ...
            if child_active_tree.schemaname == active_tree.schemaname:
                debug.deprint(
                    "Warning: recursive schema elements not supported: %s" %
                    active_tree.name)
    ...
    def set_combobox_liststore(self,
                               column,
                               cellCombo,
                               treemodel,
                               iter,
    ...
    def set_cellpicture_cardinality(self,
                                    column,
                                    cell,
                                    treemodel,
                                    iter,
    ...
    """
        ...
        if choice_or_tree.cardinality == "":
            ...
            count = parent_tree.count_children_by_schemaname(
                choice_or_tree.schemaname)
    ...
    def delete_tree(self, iter, confirm):
        ...
        if confirm:
            response = dialogs.prompt(
                self.main_window, "Are you sure you want to delete this node?")
    ...
    """
        ...
        if choice_or_tree.cardinality == "":
            ...
            iter = self.treestore.insert_after(
                self.treestore.iter_parent(iter), iter,
                [new_tree, new_tree.get_current_tree(), liststore])
            attrid = new_tree.connect("on-set-attr", self.on_set_attr,
                                      self.treestore.get_path(iter))
            dataid = new_tree.connect("on-set-data", self.on_set_data,
                                      self.treestore.get_path(iter))
    ...
    def options_tree_select_func(self,
                                 selection,
                                 model,
                                 path,
                                 is_selected,
    ...
    """
        ...
        if event.keyval == gdk.KEY_Delete:
            self.collapse_tree(self.treestore.get_iter(
    ...
    def get_spudpath(self, active_tree):
        ...
        while name_tree is not None:
            if "name" in name_tree.attrs and name_tree.attrs["name"][
                    1] is not None:
                used_name = name_tree.name + '::%s' % name_tree.attrs["name"][1]
            elif name_tree.parent is not None and name_tree.parent.count_children_by_schemaname(
                    name_tree.schemaname) > 1:
                siblings = [
                    x for x in name_tree.parent.children
                    if x.schemaname == name_tree.schemaname
                ]
                i = 0
    ...
    def get_xpath(self, active_tree):
        ...
        while name_tree is not None:
            if "name" in name_tree.attrs and name_tree.attrs["name"][
                    1] is not None:
                used_name = name_tree.name + '[@name="%s"]' % name_tree.attrs[
                    "name"][1]
            elif name_tree.parent is not None and name_tree.parent.count_children_by_schemaname(
                    name_tree.schemaname) > 1:
                siblings = [
    ...
    """
        ...
        if not isinstance(iter_or_tree, tree.Tree) and not self.iter_is_active(
                self.treestore, iter_or_tree):
            painted_tree = tree.Tree(painted_tree.name,
                                     painted_tree.schemaname,
                                     painted_tree.attrs,
                                     doc=painted_tree.doc)
            ...
            if active_tree is self.geometry_dim_tree:
                data_tree = tree.Tree(painted_tree.name,
                                      painted_tree.schemaname,
                                      datatype="fixed")
                data_tree.data = painted_tree.data
                painted_tree = MixedTree(painted_tree, data_tree)
            elif isinstance(self.geometry_dim_tree, mixedtree.MixedTree
                            ) and active_tree is self.geometry_dim_tree.parent:
                data_tree = tree.Tree(painted_tree.child.name,
                                      painted_tree.child.schemaname,
                                      datatype="fixed")
                data_tree.data = painted_tree.data
        ...
        def search(iter, node, indent=""):
            while iter:
                if self.treestore.get_value(iter, 0) is node:
                    ...
                    child = search(self.treestore.iter_children(iter), node,
                                   indent + "  ")
    ...
    """
        ...
        iter = self.get_treestore_iter_from_xmlpath("/" + self.tree.name +
                                                    self.data_paths["dim"])
        if iter is None:
            return
        ...
        if isinstance(painted_tree, mixedtree.MixedTree):
            ...
            if not isinstance(painted_tree.datatype,
                              tuple) and painted_tree.datatype != "fixed":
    ...
    def choice_or_tree_matches(self,
                               text,
                               choice_or_tree,
                               recurse,
                               search_active_subtrees=False):
        """
    ...
    """
        ...
        if choice_or_tree.is_hidden():
            ...
            if self.choice_or_tree_matches(text,
                                           choice_or_tree.get_current_tree(),
                                           False):
                return True
            elif recurse and self.find.search_gui.get_object(
                    "searchInactiveChoiceSubtreesCheckButton").get_active():
                for opt in choice_or_tree.choices():
            ...
            if self.get_painted_tree(choice_or_tree).matches(
                    text,
                    self.find.search_gui.get_object(
                        "caseSensitiveCheckButton").get_active()):
                return True
            else:
                if self.find.search_gui.get_object(
                        "caseSensitiveCheckButton").get_active():
                    text_re = re.compile(text)
                else:
                    text_re = re.compile(text, re.IGNORECASE)
                comment = choice_or_tree.get_comment()
                if comment is not None and comment.data is not None and text_re.search(
                        comment.data) is not None:
                    return True
                elif recurse:
                    for opt in choice_or_tree.children:
                        if self.choice_or_tree_matches(text, opt, recurse,
                                                       True):
                            return True
    ...
    """
        ...
        if self.choice_or_tree_matches(
                text, choice_or_tree, isinstance(choice_or_tree,
                                                 choice.Choice)):
            yield iter
...
class DiamondFindDialog:
    ...
    """
        ...
        signals = {
            "on_find_dialog_close": self.on_find_close_button,
    ...
    """
        ...
        try:
            ...
            self.parent.treeview.scroll_to_cell(path,
                                                use_align=True,
                                                col_align=0.5)
