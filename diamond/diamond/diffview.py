...
class DiffView(gtk.Window):
    ...
    def __add_controls(self):
        ...
        copyitem.add_accelerator("activate", agr, key, mod,
    ...
    def on_treeview_popup(self, treeview):
        self.show_popup(None, self.popup_location,
    ...
    def __set_treestore(self, tree, iter=None):
        ...
        child_iter = self.treestore.append(
            iter, [tree.tag, attrib, tree.text, tree.text, None])
        for child in tree:
        ...
        for edit in editscript:
            ...
            if key:
                ...
                if edit["type"] == "delete":
                    ...
                    attribs[key] = (None, old, "delete")
                    self.__insert(
                        self.__get_iter(edit["value"])[0], key + " " + old, 0)
    ...
    def __insert(self, iter, value, index):
        if " " in value:
            ...
            if before:
                self.treestore.insert_before(iter, before,
                                             [value, {}, None, None, "insert"])
            else:
    ...
    """
        ...
        if before:
            destiter = self.treestore.insert_before(
                destiter, before, [tag, attrib, text, None, "insert"])
        else:
            destiter = self.treestore.append(
                destiter, [tag, attrib, text, None, "insert"])
        ...
        def move(iterfrom, iterto):
            for childfrom in self.__iter_children(iterfrom):
                ...
                childto = self.treestore.append(
                    iterto, [tag, attrib, text, None, "insert"])
    ...
    """
        ...
        if parentiter:
            ...
            if len(siblings) != 1:
                index = "[" + str(
                    siblings.index(self.treestore.get_path(iter)) + 1) + "]"
            else:
                index = ""
    ...
    """
        ...
        attribstore = gtk.TreeStore(gobject.TYPE_PYOBJECT,
                                    gobject.TYPE_PYOBJECT,
                                    gobject.TYPE_PYOBJECT,
                                    gobject.TYPE_PYOBJECT)
