...
def find_fullset(tree):
  ...
  """
    ...
    def traverse(node):
        ...
        if node.tag == RELAXNG + "element" or (node.tag == RELAXNG + "choice"
                                               and all(
                                                   n.tag != RELAXNG + "value"
                                                   for n in node)):
            ...
            cache.add(query)
            node = tree.xpath('/t:grammar/t:define[@name="' +
                              node.get("name") + '"]',
...
def node_name(node):
  ...
  """
    tagname = node.get("name") if "name" in list(node.keys()) else strip(
        node.tag)
    name = None
