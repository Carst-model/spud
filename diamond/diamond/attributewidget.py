...
class AttributeWidget(gtk.Frame):
    ...
    """
        ...
        if not self.node.active or self.node.attrs[iter_key][
                0] is None or self.node.attrs[iter_key][0] == "fixed":
    ...
    """
        ...
        if self.node.active and isinstance(self.node.attrs[iter_key][0],
                                           tuple):
            cell_renderer.set_property("editable", True)
            cell_renderer.set_property("visible", True)
    ...
    """
        ...
        if isinstance(self.node.attrs[iter_key][0][0],
                      tuple) and self.node.attrs[iter_key][1] is None:
    ...
    """
        ...
        value_check = self.node.validity_check(self.node.attrs[iter_key][0],
                                               new_text)

        if value_check is not None and value_check != self.node.attrs[
                iter_key][1]:
    ...
    """
        ...
        if isinstance(
                self.node.attrs[iter_key][0][0],
                tuple) and new_text not in self.node.attrs[iter_key][0][0]:
            if self.node.get_attr(iter_key) is None and new_text == "":
                return

            new_text = self.node.validity_check(
                self.node.attrs[iter_key][0][1], new_text)
