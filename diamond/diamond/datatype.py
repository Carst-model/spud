...
def print_type(datatype, bracket=True):
  ...
  """
    ...
    """
        ...
        value_type_split2 = value_type_split1[len(value_type_split1) -
                                              1].split(" ")
        if len(value_type_split2) == 1:
            return familiar_type(
                value_type_split2[0][0:len(value_type_split2[0]) - 6])
        else:
    ...
    if isinstance(datatype, plist.List):
        if (isinstance(datatype.cardinality, int)
                and datatype.cardinality == 1) or datatype.cardinality == "":
            ...
            if isinstance(datatype.cardinality, int):
                type_as_printable += str(
                    datatype.cardinality) + " " + list_type_as_printable + "s"
            else:
