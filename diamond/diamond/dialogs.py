...
def prompt(parent, message, type=gtk.MessageType.QUESTION, has_cancel=False):
  ...
  """
    ...
    prompt_dialog = gtk.MessageDialog(transient_for=parent,
                                      flags=0,
                                      message_type=type,
                                      buttons=gtk.ButtonsType.NONE,
                                      text=message)
    prompt_dialog.add_buttons(gtk.STOCK_YES, gtk.ResponseType.YES,
...
def long_message(parent, message):
  ...
  """
    ...
    message_dialog = gtk.Dialog(parent=parent,
...
def error(parent, message):
  ...
  """
    ...
    error_dialog = gtk.MessageDialog(parent, 0, gtk.MessageType.WARNING,
...
def get_filename(parent,
  ...
  """
    ...
    if action == gtk.FileChooserAction.SAVE:
        ...
        buttons = (gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL, gtk.STOCK_NEW,
                   gtk.ResponseType.OK)
    else:
        buttons = (gtk.STOCK_CANCEL, gtk.ResponseType.CANCEL, gtk.STOCK_OPEN,
                   gtk.ResponseType.OK)
    filew = gtk.FileChooserDialog(title=title,
                                  parent=parent,
                                  action=action,
                                  buttons=buttons)
...
def console(parent, locals=None):
  ...
  """
    ...
    console_dialog = gtk.Dialog(parent=parent,
                                buttons=(gtk.STOCK_QUIT,
                                         gtk.ResponseType.ACCEPT))
...
class GoToDialog:
    ...
    def run(self):
        signals = {
            "goto_activate": self.on_goto_activate,
