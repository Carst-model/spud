...
plugin_dirs = [
    dir for dir in os.listdir('plugins')
    if os.path.isdir(os.path.join('plugins', dir)) and dir[0] != '.'
]
plugin_data_files = []
if sys.platform == 'darwin' and packaging:
    for plugin in plugin_dirs:
        plugin_data_files.append(
            ("./plugins/" + plugin, glob.glob('plugins/' + plugin + '/*.py')))
else:
    for plugin in plugin_dirs:
        if prefix is None:
            plugin_data_files.append(
...
if sys.platform == 'darwin' and packaging:
    ...
    if prefix is None:
        gui_data_files.append(
            ("@prefix@/share/diamond/gui", ["gui/gui.ui", "gui/diamond.svg"]))
    else:
        gui_data_files.append(
            (prefix + "/share/diamond/gui", ["gui/gui.ui", "gui/diamond.svg"]))
